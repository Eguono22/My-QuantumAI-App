...
const weeklyPlan = [
  ...
  {
    label: 'Days 8-15',
    title: 'Prove trust and execution',
    detail: 'Review order behavior, trust feedback, and willingness to pay. Look for repeated proof, not one-off reactions.',
  },
  {
    label: 'Days 16-30',
    title: 'Scale only if the proof holds',
    detail: 'Use the longer pilot window to prove repeatability, scaling safety, and final release readiness.',
  },
];
...
function buildCheckpointCard({
  ...
  const state = !reached ? 'progress' : pass ? 'pass' : 'hold';
  const statusMap = {
    pass: 'PASS',
    hold: 'HOLD',
...
function buildDayEightToThirtyCheckpoints({
  currentDay,
  orders,
  outcomeConsistency,
  outcomeTrend,
  ...
  for (let day = 8; day <= 30; day += 1) {
    if (day <= 10) {
      const requiredOrders = day - 3;
      const reached = currentDay >= day || orders.length >= requiredOrders;
      const pass = orders.length >= requiredOrders
        && outcomeConsistency.sampleSize >= requiredOrders
        && outcomeConsistency.fillRate >= 40
        && executionConfidence.score >= 55;
      const remainingOrders = Math.max(0, requiredOrders - orders.length);

      checkpoints.push(buildCheckpointCard({
        day,
        reached,
        pass,
        titles: {
          pass: `Day ${day} execution-stability check passed`,
          hold: `Day ${day} execution-stability check is on hold`,
          progress: `Day ${day} execution-stability check not reached yet`,
        },
        messages: {
          pass: 'Execution behavior is stable enough to support trust-focused sessions.',
          hold: 'Execution behavior still lacks enough consistency for trend-based trust claims.',
          progress: `Need ${remainingOrders} more recorded order${remainingOrders === 1 ? '' : 's'} to judge trend stability.`,
        },
        nextActions: {
          pass: 'Keep collecting outcomes and compare pattern shifts daily through Day 10.',
          hold: 'Increase tiny paper order coverage and reduce noisy rejects before claiming stability.',
          progress: 'Capture a broader sample of order outcomes before making trend claims.',
        },
        metrics: {
          pass: [
    ...
    if (day <= 15) {
      const requiredFeedback = day - 6;
      const reached = currentDay >= day || feedbackTotal >= requiredFeedback;
      const pass = feedbackTotal >= requiredFeedback
        && avgTrust >= 4
        && avgValue >= 4
        && yesRate >= 40
        && releaseGateDecision.status === 'READY';
      const remainingFeedback = Math.max(0, requiredFeedback - feedbackTotal);
      ...
      checkpoints.push(buildCheckpointCard({
        ...
        pass,
        titles: {
          pass: `Day ${day} trust-and-value checkpoint passed`,
          hold: `Day ${day} trust-and-value checkpoint is on hold`,
          progress: `Day ${day} trust-and-value checkpoint not reached yet`,
        },
        messages: {
          pass: 'Trust, value, and willingness-to-pay proof are strong enough to support expansion.',
          hold: 'Trust confidence is still below threshold for a safe pilot expansion decision.',
          progress: `Need ${remainingFeedback} more feedback entr${remainingFeedback === 1 ? 'y' : 'ies'} before this checkpoint is meaningful.`,
        },
        nextActions: {
          pass: 'Preserve the same trust framing and keep the release gate unchanged.',
          hold: 'Address the trust gap before widening the pilot audience.',
          progress: 'Keep collecting trust-focused interviews and note the recurring friction points.',
        },
        metrics: {
          pass: [
    ...
    if (day <= 20) {
      const requiredFeedback = day - 5;
      const reached = currentDay >= day || feedbackTotal >= requiredFeedback || completedCandidates >= 4;
      const pass = feedbackTotal >= requiredFeedback
      ...
      checkpoints.push(buildCheckpointCard({
        ...
        pass,
        titles: {
          pass: `Day ${day} repeatability checkpoint passed`,
          hold: `Day ${day} repeatability checkpoint is on hold`,
          progress: `Day ${day} repeatability checkpoint not reached yet`,
        },
        messages: {
          pass: 'The workflow is repeatable enough that users can follow it without extra help.',
          hold: 'Repeatability still needs more completed sessions and consistent feedback.',
          progress: `Need ${remainingFeedback} more feedback entr${remainingFeedback === 1 ? 'y' : 'ies'} and more completed users before repeatability is meaningful.`,
        },
        nextActions: {
          pass: 'Keep the walkthrough stable and preserve the same session script.',
          hold: 'Standardize the walkthrough and remove the most common confusion point.',
          progress: 'Continue collecting completed sessions and compare whether users need help twice.',
        },
        metrics: {
          pass: [
            `Completed candidates: ${completedCandidates}`,
            `Feedback entries: ${feedbackTotal}`,
            `Execution confidence: ${executionConfidence.score}/100`,
          ],
          hold: [
            `Completed candidates: ${completedCandidates}`,
            `Feedback entries: ${feedbackTotal}`,
    ...
    if (day <= 25) {
      const requiredFeedback = day - 4;
      const reached = currentDay >= day || feedbackTotal >= requiredFeedback || orders.length >= 10;
      const pass = feedbackTotal >= requiredFeedback
        && orders.length >= 10
        && yesRate >= 45
      ...
      checkpoints.push(buildCheckpointCard({
        ...
        pass,
        titles: {
          pass: `Day ${day} scale-readiness checkpoint passed`,
          hold: `Day ${day} scale-readiness checkpoint is on hold`,
          progress: `Day ${day} scale-readiness checkpoint not reached yet`,
        },
        messages: {
          pass: 'Evidence supports inviting a larger paper-only cohort.',
          hold: 'Scaling still risks amplifying the current blockers.',
          progress: `Need ${remainingFeedback} more feedback entr${remainingFeedback === 1 ? 'y' : 'ies'} and a larger order sample before scaling is meaningful.`,
        },
        nextActions: {
          pass: 'Keep the expansion script simple and preserve the current trust framing.',
          hold: 'Do not widen the cohort until execution and trust remain stable.',
          progress: 'Continue logging proof points before considering expansion.',
        },
        metrics: {
          pass: [
    ...
    const reached = currentDay >= day || feedbackTotal >= 15 || releaseGateDecision.status === 'READY';
    const pass = day === 30
      ? releaseGateDecision.status === 'READY'
    ...
    checkpoints.push(buildCheckpointCard({
      ...
      titles: {
        pass: day === 30
          ? 'Day 30 final release-readiness check passed'
          : `Day ${day} release-candidate checkpoint passed`,
        hold: day === 30
          ? 'Day 30 final release-readiness check is on hold'
          : `Day ${day} release-candidate checkpoint is on hold`,
        progress: day === 30
          ? 'Day 30 final release-readiness check not reached yet'
          : `Day ${day} release-candidate checkpoint not reached yet`,
      },
      messages: {
        pass: day === 30
          ? 'Pilot evidence across execution, trust, value, and pricing supports controlled expansion.'
          : 'The release-candidate proof is strong enough to keep moving toward final readiness.',
        hold: day === 30
          ? 'One or more pilot checkpoints are still below threshold for expansion.'
          : 'Release-candidate proof still needs more stability before the final call.',
        progress: day === 30
          ? 'Complete the longer proof set before making the final release call.'
          : 'Keep collecting proof before treating this checkpoint as valid.',
      },
      nextActions: {
        pass: day === 30
          ? 'Expand to the next cohort while preserving existing safety and reliability controls.'
          : 'Use the stronger release-candidate evidence to keep hardening the pilot.',
        hold: day === 30
          ? 'Resolve remaining hold checkpoints before opening the next beta cohort.'
          : 'Keep hardening the pilot until the release-candidate proof is stable.',
        progress: day === 30
          ? 'Continue the pilot script and keep collecting evidence daily.'
          : 'Continue collecting evidence until the final readiness sample is valid.',
      },
      metrics: {
        pass: [
...
function buildPilotReport({
  ...
  return [
    '# QuantumAI 30-Day Trust Pilot Report',
    '',
    ...
    '',
    ...dayEightToThirty.flatMap((checkpoint) => [
...
export default function Pilot() {
  ...
  const dayEightToThirty = buildDayEightToThirtyCheckpoints({
    currentDay,
    orders,
    outcomeConsistency,
    outcomeTrend,
